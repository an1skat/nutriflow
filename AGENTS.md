...
- **Окремий акаунт кожної школи.** Школа логіниться логіном/паролем і потрапляє на першу вкладку — **тижневе меню**. У відкритому денному меню може додавати, замінювати й видаляти страви, але не може зберегти порожній день. Страви мають бути «валідними» (прив'язуються до карток страв, але не надто суворо).
