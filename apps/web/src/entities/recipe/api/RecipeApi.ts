...
export async function fetchDishCards(
  query?: string,
): Promise<DishCardList> {
...
export async function fetchDishCard(
  dishCardId: string,
): Promise<DishCard> {
  const response = await apiClient.get<unknown>(
    `/recipes/dish-cards/${dishCardId}`,
  );
  return dishCardSchema.parse(response.data);
}

export async function fetchDishCardVersions(
  dishCardId: string,
): Promise<DishCardVersionList> {
  const response = await apiClient.get<unknown>(
...
export async function fetchDishCardVersion(
  versionId: string,
...
export async function createDishCard(
  payload: CreateDishCardPayload,
): Promise<DishCard> {
...
export async function updateIngredient(
  ...
  const response = await apiClient.patch<unknown>(
    `/recipes/ingredients/${ingredientId}`,
    payload,
    { headers: getCsrfHeaders() },
  );
  return ingredientSchema.parse(response.data);
...
export async function validateDishCardVersion(
  versionId: string,
...
export async function confirmDishCardVersion(
  versionId: string,
): Promise<DishCardVersion> {
  const response = await apiClient.post<unknown>(
