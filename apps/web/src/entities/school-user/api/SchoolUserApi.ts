...
export async function fetchSchoolUsers(
  ...
  const response = await apiClient.get<unknown>(`/admin/schools/${schoolId}/users`, {
    params: toApiPaginationParams(request),
  });
  return schoolUserListSchema.parse(response.data);
}

export async function fetchSchoolUser(schoolId: string, userId: string): Promise<SchoolUser> {
  const response = await apiClient.get<unknown>(`/admin/schools/${schoolId}/users/${userId}`);
...
export async function createSchoolUser(
  ...
  const response = await apiClient.post<unknown>(`/admin/schools/${schoolId}/users`, payload, {
...
export async function deleteSchoolUser(schoolId: string, userId: string): Promise<void> {
