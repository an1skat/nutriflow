...
import { fetchAdminUser, fetchAdminUsers } from './AdminUserApi';

export const adminUserQueryKeys = {
  all: ['protected', 'admin', 'admins'] as const,
  lists: () => [...adminUserQueryKeys.all, 'list'] as const,
  list: (request: PageRequest) => [...adminUserQueryKeys.lists(), request] as const,
  detail: (userId: string) => [...adminUserQueryKeys.all, 'detail', userId] as const,
};
