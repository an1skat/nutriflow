...
export function WeeklyMenuExcelTools({
  ...
  const safeActiveSheet =
    activeSheet < (preview?.menus.length ?? 0) ? activeSheet : 0;
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      toast.success(
        `Імпорт завершено. Створено тижневих меню: ${result.created_menu_ids.length}.`,
      );
      setFile(null);
      setPreview(null);
      setPreviewOpen(false);
          ...
          <div className="space-y-4 border-t border-slate-300 pt-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-600">
                  {previewSummary?.menus ?? 0} тиж. · {previewSummary?.days ?? 0} дн. · {previewSummary?.items ?? 0} позицій · помилок: {errors.length} · попереджень: {warnings.length}
                </p>
            ...
            {diagnostics.length ? (
              <div className="border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-950">
                Є зауваження до файлу: помилок {errors.length}, попереджень{" "}
                {warnings.length}. Деталі доступні у preview.
              </div>
