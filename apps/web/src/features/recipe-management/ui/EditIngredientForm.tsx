...
export function EditIngredientForm({ ingredient, onSaved }: EditIngredientFormProps) {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    try {
      await update.mutateAsync({ id: ingredient.id, payload: values });
      onSaved?.();
    } catch (error) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-[1fr_100px]">
        <div>
          <label htmlFor="edit-ingredient-name" className="nf-label">Назва</label>
        ...
        <div>
          <label htmlFor="edit-ingredient-unit" className="nf-label">Одиниця</label>
      ...
      <div>
        <label htmlFor="edit-ingredient-aliases" className="nf-label">Аліаси (через кому)</label>
        <input
