...
import { validateDishCardVersion } from '@/entities/recipe/api/RecipeApi';
  ...
  return {
    ...
    ingredients: Array.from(
      ...
      for (const a of version.ingredient_amounts) {
        if (a.ingredient_name_snapshot === row.ingredient_name_snapshot) {
...
export function DishCardVersionForm({
  ...
  const selectedAllergenIds = useWatch({ control: form.control, name: 'allergen_ids' }) ?? [];
      ...
      <div className="nf-panel">
        <div className="nf-panel-body">
          <p className="text-sm text-slate-700">
            Версія має статус <b>{version!.status}</b> і є незмінною. Створіть нову версію, щоб
            внести правки.
          </p>
          <Link
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      ...
      <Section title="Технологія">
        <textarea className="nf-input min-h-24" rows={4} {...form.register('technology_text')} />
      </Section>
            ...
            return (
              <div key={field.id} className="border border-slate-200 bg-white p-3">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <p className="m-0 text-xs font-bold text-slate-700">Порція {index + 1}</p>
                  {nutritionBlank ? (
                ...
                <div className="grid gap-3 sm:grid-cols-[120px_repeat(4,1fr)_auto]">
                  <Field label="Маса, г" error={portionError?.portion_grams?.message}>
                    <input
                      className="nf-input"
                      placeholder="120"
                  ...
                  <Field label="Жири" error={portionError?.fats?.message as string | undefined}>
                    <input
                      className="nf-input"
                      placeholder="0"
                  ...
                  <Field label="ккал" error={portionError?.kcal?.message as string | undefined}>
                    <input
                      className="nf-input"
                      placeholder="0"
