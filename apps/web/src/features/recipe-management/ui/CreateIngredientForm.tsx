...
export function CreateIngredientForm({ onCreated }: { onCreated?: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-3 sm:grid-cols-[1fr_100px_auto]">
      <div>
        <label htmlFor="ingredient-name" className="nf-label">Назва</label>
      ...
      <div>
        <label htmlFor="ingredient-unit" className="nf-label">Одиниця</label>
        <input id="ingredient-unit" className="nf-input" {...form.register("unit")} />
        {form.formState.errors.unit ? (
          <p role="alert" className="nf-field-error">{form.formState.errors.unit.message}</p>
        ) : null}
      </div>
      <div className="flex flex-col">
        <span className="nf-label invisible hidden sm:block" aria-hidden="true">&nbsp;</span>
      ...
      <div className="sm:col-span-3">
        <label htmlFor="ingredient-aliases" className="nf-label">Аліаси (через кому)</label>
        <input id="ingredient-aliases" className="nf-input" placeholder="морква, морква свіжа" {...form.register("aliases")} />
      </div>
      {form.formState.errors.root ? (
        <p role="alert" className="nf-error sm:col-span-3">{form.formState.errors.root.message}</p>
      ) : null}
    </form>
  );
}
