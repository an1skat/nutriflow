...
import { NormComplianceReportView, NormComplianceWorkspace } from './NormComplianceWorkspace';
...
describe('NormComplianceReportView', () => {
  it('renders unmapped, missing, and stale states and explains a selected row', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Не визначено нормативну групу/ })
    ).toBeInTheDocument();
    ...
    expect(onSelectRow).toHaveBeenCalledWith(
      expect.objectContaining({
        row: expect.objectContaining({ normative_group_code: 'vegetables' }),
      })
    );
  });
  ...
  it('opens the contribution explanation when a row is selected', () => {
    ...
    expect(
      within(dialog).getByText('Застаріла меню-вимога:', { exact: false })
    ).toBeInTheDocument();
...
describe('NormComplianceWorkspace', () => {
  ...
  it('uses the concrete week transferred by the calendar without editable filters', () => {
    ...
    expect(screen.getAllByRole('heading', { name: /06 лип.*10 лип/ })).toHaveLength(2);
  ...
  it('blocks direct report access without the menu-requirements calendar source', () => {
    ...
    expect(screen.getByText('Звіт формується тільки з календаря')).toBeInTheDocument();
