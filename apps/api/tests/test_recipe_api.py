...
def test_alternative_group_requires_explicit_choice(admin_client):
    client, _ = admin_client
    ...
    assert "Alternative choice required" in response.json()["detail"]


def test_selected_alternative_is_not_summed_with_other_options(admin_client):
    client, _ = admin_client
    variant_id, version_id = _prepare_alternative_group_card(client)
    ...
    assert len(items) == 1
    assert items[0]["ingredient_name_snapshot"] == "Морква свіжа з 01.01"
    assert items[0]["gross_total"] == "1060.80"
...
def test_school_user_cannot_read_recipe_catalog(school_client):
    """Read access is currently admin-only as agreed in the spec."""
    client, _ = school_client
    assert client.get("/api/v1/recipes/ingredients").status_code == 403
    assert client.get("/api/v1/recipes/dish-cards").status_code == 403
